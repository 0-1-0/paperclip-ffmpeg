...
describe 'Mobile' do
  ...
  it 'handles rotation and portrait-landscape shananigans and gives adequate preview (ladnscape with strange rotation)' do
  ...
  it 'forcing aspect ratio by cropping image with AxB# parameter (landscape with strange rotation)' do
    @video = Video.create(mobile: File.new(Dir.pwd + '/spec/support/landscape.mp4'))
    file = File.open(Dir.pwd + '/spec/dummy/public' + @video.mobile(:pad).split('?')[0])
